...
- **patching** — activation extract / inject / logits / relevancy.
- **helpers** — config loading, prompt formatting, answer parsing,
  placeholder resolution, interpretation-prompt construction.
- **evaluation** — aggregate metric computation.
- **source_overrides** — Phase 3 raw-text override trials.
- **reporting** — companion .txt log writer.
...
from experiments.patchscope import (
    evaluation,
    helpers,
    patching,
    reporting,
...
class PatchscopeExperiment(BaseExperiment):
    ...
    def setup(self) -> None:
        ...
        self.ps_config = helpers._load_patchscope_config(self.config.patchscope_config)
        ...
        if self._source_mode == "manual":
            ...
            self.questions = helpers._load_questions(
                task_file,
    ...
    def _run_filename_suffix(self) -> str:
        ...
        try:
            _styles = helpers.normalize_prompt_styles(ps)
        except ValueError:
            _styles = [
                str(
                    ps.get("prompt_style")
    ...
    def _init_output_paths(self) -> None:
        ...
        model_short = helpers._model_short_name(self.config.model_name)
    ...
    def run(self) -> None:
        ...
        templates = helpers.effective_interpretation_templates(ps_config)
        ...
        for _tn, _tc in templates.items():
            _probes = helpers.resolve_prompt_styles_for_tmpl_cfg(_tc, ps_config)
            if not _probes:
                raise ValueError(
        ...
        # Resolve layer pairs
        source_layers, injection_layers, pair_map = helpers._resolve_layer_config(
        ...
        placeholder_token_id = helpers._get_placeholder_token_id(tokenizer, configured_placeholder)
        ...
        for tmpl_name, tmpl_cfg in templates.items():
            if tmpl_cfg.get("decode_mode", "generate") == "logits" and tmpl_name in helpers.TEMPLATE_CHOICES:
                all_choice_token_ids[tmpl_name] = helpers._resolve_choice_token_ids(
                    tokenizer, helpers.TEMPLATE_CHOICES[tmpl_name]
                )
        ...
        _use_both = helpers.extraction_uses_both_modes(extraction_cfg)
        _n_probes = sum(
            len(helpers.resolve_prompt_styles_for_tmpl_cfg(tc, ps_config))
        ...
        if source_override_list:
            source_overrides.run_source_overrides(
                ...
                try:
                    ...
                    if _is_manual:
                        ...
                        for _tw in _target_words:
                            _pos = helpers.find_token_position(
                        ...
                        _q_acts: dict = {}
                        _multi_prefill = patching.extract_activations_multi_pos(
                            model, tokenizer, device,
                            messages=[],
                        ...
                        _ar_meta = None
                        if _n_gen_tokens > 0:
                            _multi_gen, _ar_meta = patching.extract_activations_during_decode_multi_step(
                                model, tokenizer, device,
                                messages=[],
                                layer_indices=source_layers,
                        ...
                        shuffled_question, remap = helpers._shuffle_options(
                        ...
                        user_msg = helpers.format_source_pass_user_message(
                        ...
                        _use_both = helpers.extraction_uses_both_modes(extraction_cfg)
                        _wants_gen_capture = _use_both or helpers.extraction_uses_generation_time_capture(
                            extraction_cfg
                        )
                        ...
                        if not hasattr(self, '_extraction_validated'):
                            ...
                            if use_prefill_extract:
                                helpers.validate_extraction_position(
                        ...
                        if source_raw_text is not None:
                            ...
                            _source_token_ids = (
                                patching._normalize_chat_template_token_ids(
                                    _ct_result
                                )
                            )
                        ...
                        if _use_multi_pos:
                            ...
                            if use_prefill_extract:
                                _prefill_caps = (
                                    patching.extract_activations_multi_layer(
                                        model, tokenizer, device, messages,
                                        layer_indices=source_layers,
                            ...
                            if use_ar_extract:
                                _ar_caps, _ar_meta = (
                                    patching.extract_activations_during_decode(
                                        model,
                                        tokenizer,
                                        device,
                                        messages,
                            ...
                            def _prefill_meta() -> dict:
                                site = helpers.describe_source_extraction_site(
                        ...
                        if sp_name not in self._sample_source_prompts:
                            ...
                            if use_ar_extract:
                                ...
                                _site = helpers.describe_source_extraction_site(
                                    tokenizer,
                                    _ptext,
                    ...
                    for src_layer in source_layers:
                        ...
                        for inj_layer in paired_inj_layers:
                            for reporter_name in reporter_persona_names:
                                ...
                                for tmpl_name, tmpl_cfg in templates.items():
                                    for prompt_style in (
                                        helpers.resolve_prompt_styles_for_tmpl_cfg(
                                        ...
                                        interp_text, interp_messages, placeholder_positions = (
                                            helpers.build_interpretation_prompt(
                                                tokenizer=tokenizer,
                                                tmpl_cfg=tmpl_cfg,
                                        ...
                                        if _vk not in self._placeholder_validation_done:
                                            helpers.validate_placeholder_positions(
    ...
    def _run_single_cell(
        ...
        result = patching.patch_and_decode(
            model, tokenizer, device,
        ...
        if use_logits:
            ...
            if tmpl_name in helpers.TEMPLATE_CHOICES:
                record.reporter_parsed_answer, record.reporter_parse_success = (
                    helpers._parse_constrained(tmpl_name, result["generated_text"])
                )
            else:
            ...
            if (
                ...
                if gen_token_ids:
                    rel_scores = patching.compute_relevancy_scores(
                        model, tokenizer, device,
                        interp_messages, activation,
                        injection_layer=inj_layer,
                        placeholder_positions=placeholder_positions,
        ...
        if helpers.reporter_sample_opposing_qualifies(
    ...
    def _capture_no_persona_plain_prompt_per_layer(
        ...
        *,
        ...
        ns_text, ns_msgs, ns_ph = helpers.build_interpretation_prompt(
            tokenizer=tokenizer,
    ...
    def evaluate(self) -> dict:
        ...
        return evaluation.compute_metrics(self.records)
