...
Usage:
    python -m experiments.patchscope.run_patchscope_test
    python -m experiments.patchscope.run_patchscope_test --config patchscope_test.yaml
    python -m experiments.patchscope.run_patchscope_test --model meta-llama/Llama-3.1-8B-Instruct
"""
...
CONFIG_DIR = Path(__file__).resolve().parent  # patchscope-specific configs colocated
RESULTS_DIR = ROOT / "results" / "raw"
RESULTS_DIR.mkdir(parents=True, exist_ok=True)

# Reuse the core hook functions from the patchscope experiment
sys.path.insert(0, str(ROOT))
from experiments.patchscope.helpers import (
