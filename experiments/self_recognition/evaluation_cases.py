...
class CaseSpec:
    ...
    def answer_keys(self) -> tuple[str, str]:
        ...
        if self.roles == "pair_described":
            # attribute-both (cases 8/9) vs pick-one "which is the first persona's?" (13/14)
            return ("t1p1_t2p2", "t1p2_t2p1") if self.answer_form == "attribution" else ("text1", "text2")
        if self.n_texts == 1:
...
_OPT_WHICH_TEXT_P1 = {  # two described personas, PICK which text is the FIRST persona's (cases 13/14)
    "text1": "Text 1 was generated under the first persona prompt.",
    "text2": "Text 2 was generated under the first persona prompt.",
}
...
CASE_REGISTRY: dict[str, CaseSpec] = {
    ...
    "case13": CaseSpec(
        case_id="case13", case_type="baseline", roles="pair_described", n_texts=2,
        answer_form="single", eval_sp=(False,), describe="both",
        description_style="system_prompt_style", self_is_candidate=True,
        intro=("Below are two pieces of text generated by AI assistants in response to "
               "the same task. One was generated under the first persona prompt and the "
               "other under the second persona prompt, both described below. Which text "
               "was generated under the first persona prompt?"),
        option_sentences=_OPT_WHICH_TEXT_P1,
        note="framing-matched neutral FULL-description control — pick-one twin of case 8"),
    "case14": CaseSpec(
        case_id="case14", case_type="baseline", roles="pair_described", n_texts=2,
        answer_form="single", eval_sp=(False,), describe="both",
        description_style="redacted", self_is_candidate=True,
        intro=("Below are two pieces of text generated by AI assistants in response to "
               "the same task. One was generated under the first persona prompt and the "
               ...
               "was generated under the first persona prompt?"),
        option_sentences=_OPT_WHICH_TEXT_P1,
        note="framing-matched STYLE-ONLY control — pick-one twin of case 9 (redacted)"),
}
...
def _enumerate_case(spec: CaseSpec, group: str, keys: list[str],
    ...
    elif spec.roles == "pair_described" and spec.self_is_candidate and spec.answer_form == "single":
        # cases 13, 14 — neutral, both described, PICK which text is the FIRST persona's
        # (persona_1 = E). Same structure as cases 8/9 but pick-one, not attribute-both.
        for ev in personas:
            for other in personas:
                if other == ev:
                    continue
                ...
                for task in keys:
                    for to in _TEXT_ORDERS_PAIR:
                        ...
                        for ao in aorders:
                            am = _mapping(akeys, ao)
                            truth = "text1" if t1 == p1 else "text2"
                            out.append(mk(
                                ev, task, correct=_letter_for(am, truth), answer_mapping=am,
